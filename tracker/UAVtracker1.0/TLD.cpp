...
void TLD::processFrame(const cv::Mat& frame, const cv::Mat& img1, const cv::Mat& img2, BoundingBox& bbnext, bool& lastboxfound, bool tl, bool tk, FILE* bb_file)
{
	...
	clock_t t1 = clock();
	detect(img2, lastboxfound, tk);
	if (tconf > 0.65&&tracked == true)
		detected = false;
...
void TLD::detect(const cv::Mat& frame,bool lastboxfound,bool tk){
  //cleaning
  dbb.clear();
  dconf.clear();
